...
class V2CaptureCoordinator {
  ...
  final Map<String, _ActiveV2Session> _activeSessions = {};
  ...
  Future<SessionJournalRow> startTracking({
    required String tripId,
    String? timezone,
  ...
  Future<SessionJournalRow?> pauseTracking({
    required String tripId,
  }) async {
    ...
    _activeSessions.remove(paused.sessionId);
    await _syncCaptureSubscription();
    return paused;
  }

  Future<SessionJournalRow?> resumeTracking({
    required String tripId,
  }) async {
    await _requireLocationAccess(requestIfDenied: true);
  ...
  Future<SessionJournalRow?> stopTracking({
    required String tripId,
  }) async {
  ...
  Future<int> recoverAndEnforceSingleActiveSession() async {
    ...
    final keptCount = await _repository.recoverAndEnforceSingleActiveSession();
    final activeSessions = await _repository.listActiveSessions();
    if (keptCount == 0 || activeSessions.isEmpty) {
    ...
    _activeSessions
      ..clear()
      ..[active.sessionId] = _ActiveV2Session(
        sessionId: active.sessionId,
        tripId: active.tripLocalId,
      );
    await _syncCaptureSubscription();
    return 1;
  }
  ...
  Future<void> _setSingleActiveSession(SessionJournalRow session) async {
    _activeSessions
      ..clear()
      ..[session.sessionId] = _ActiveV2Session(
        sessionId: session.sessionId,
        tripId: session.tripLocalId,
      );
  }
  ...
  Future<T> _runCommand<T>(Future<T> Function() action) async {
    try {
      return await action();
    } on V2LiveTrackingCommandException catch (error) {
...
class _ActiveV2Session {
  const _ActiveV2Session({
